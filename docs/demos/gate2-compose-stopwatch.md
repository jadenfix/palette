...
Status: stale pre-hardening maintainer diagnostic. The browser recording from
this run was 3.68s long and did not satisfy the current 8-second reviewability
floor, so the stale binary has been removed from the canonical
`docs/demos/gate2-compose-browser-demo.webm` evidence path. This file must be
regenerated by the default-port compose stopwatch path before it can support
Gate 2 evidence.
...
- Browser recording artifact: `docs/demos/gate2-compose-browser-demo.webm` (pending regenerated recording)
- Browser recording notes: `docs/demos/gate2-compose-browser-demo.md`
- Browser recording SHA256: pending regenerated recording
