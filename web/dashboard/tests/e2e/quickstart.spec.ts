...
test("renders the five-line stock OTLP quickstart trace in a browser", async ({ page }) => {
  ...
  await form.getByLabel("Reason").fill("gate2-quickstart-review");
  await form.getByRole("button", { name: "Unmask" }).click();
  await expect(page).toHaveURL(/unmask=true/);
  await expect(page).toHaveURL(/reason=gate2-quickstart-review/);
  await expect(detail).toContainText("Unmask requested");
  await expect(detail).toContainText("gate2-quickstart-review");
  await expect(promptIo.locator("pre")).toHaveText("hello from stock OpenTelemetry");
  await expect(completionIo.locator("pre")).toHaveText("hello from Beater");
});
