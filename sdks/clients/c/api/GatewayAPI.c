...
GatewayAPI_gatewayChatCompletions(apiClient_t *apiClient, char *tenant_id, char *project_id, char *environment_id, gateway_chat_http_request_t *gateway_chat_http_request, char *authorization, char *x_beater_api_key, char *x_beater_project_id, char *x_beater_environment_id)
{
    ...
    // create the path
    char *localVarPath = strdup("/v1/gateway/{tenant_id}/{project_id}/{environment_id}/chat/completions");
    ...
    // Body Param
    cJSON *localVarSingleItemJSON_gateway_chat_http_request = NULL;
    if (gateway_chat_http_request != NULL)
    {
        //not string, not binary
        localVarSingleItemJSON_gateway_chat_http_request = gateway_chat_http_request_convertToJSON(gateway_chat_http_request);
        localVarBodyParameters = cJSON_Print(localVarSingleItemJSON_gateway_chat_http_request);
    ...
    //    printf("%s\n","Proxied OpenAI-compatible chat completion");
    //}
    // uncomment below to debug the error response
    //if (apiClient->response_code == 400) {
    //    printf("%s\n","Invalid request, scope, or no key + no managed default");
    //}
    // uncomment below to debug the error response
    ...
    //    printf("%s\n","Provider secret not found or inactive");
    //}
    // uncomment below to debug the error response
    //if (apiClient->response_code == 502) {
    //    printf("%s\n","All providers/keys failed");
    //}
    // uncomment below to debug the error response
    ...
    gateway_outcome_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *GatewayAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = gateway_outcome_parseFromJSON(GatewayAPIlocalVarJSON);
        cJSON_Delete(GatewayAPIlocalVarJSON);
    ...
    if (localVarSingleItemJSON_gateway_chat_http_request) {
        cJSON_Delete(localVarSingleItemJSON_gateway_chat_http_request);
        localVarSingleItemJSON_gateway_chat_http_request = NULL;
    }
